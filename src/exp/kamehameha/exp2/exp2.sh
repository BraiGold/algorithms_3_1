...
while getopts 'ha:' opt; do
  case $opt in
    ...
    h) echo ""
       echo "    Experimento 2 - Tiempo de ejecución en casos borde"
       echo ""
...
printf "%d \n" $iteraciones >> $(dirname $0)/tiempos-exp2.txt
  ...
  for h in $(seq 1 $iteraciones); do
     $(dirname $0)/../../../kamehameha < $(dirname $0)/puntosAlineados-$n.txt -t >> $(dirname $0)/tiempos-exp2.txt
  done
  printf "\n" >> $(dirname $0)/tiempos-exp2.txt

 printf "%d " 2 >> $(dirname $0)/tiempos-exp2.txt
  for h in $(seq 1 $iteraciones); do
    echo "estoy corriendo la iteracion numero"
    printf "%d\n" $h
     $(dirname $0)/../../../kamehameha < $(dirname $0)/puntosNOalineados-$n.txt -t >> $(dirname $0)/tiempos-exp2.txt
  done
  printf "\n" >> $(dirname $0)/tiempos-exp2.txt
